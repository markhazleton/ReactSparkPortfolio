...
const VariantList: React.FC = () => {
  ...
  const enhancedVariants: EnhancedVariant[] = React.useMemo(() => {
    ...
    return rawVariants.map((variant): EnhancedVariant => {
      ...
      return {
        ...
        description:
          variant.description || `Chat with the ${variant.name} variant of PromptSpark AI.`,
      };
    });
  }, [rawVariants]);
  ...
  const categories = React.useMemo(() => {
    ...
    const uniqueCategories = Array.from(
      new Set(["all", ...enhancedVariants.map((v) => v.category)])
    );
      ...
      <div className="row mb-4">
        ...
        <div className="col-lg-6">
          <InputGroup className="mb-3">
            <InputGroup.Text
              className={isDark ? "bg-dark text-light border-secondary" : "bg-light"}
            >
              <Search />
            </InputGroup.Text>
            <Form.Control
                  ...
                  <div className="col" key={`featured-${variant.id || index}`}>
                    ...
                    >
                      <Card.Body>
                        ...
                        <Card.Text
                          className={isDark ? "text-light-emphasis" : "text-body-secondary"}
                        >
                          {variant.description}
                        </Card.Text>
                      </Card.Body>
          ...
          <div key="filtered-variants" className="mb-4">
            {activeCategory !== "all" || searchTerm ? (
              <h3 className="h5 mb-3">
                ...
                {filteredVariants.length === 1 ? "Assistant" : "Assistants"} Found
              </h3>
            ) : (
            ...
            {filteredVariants.length === 0 ? (
              <Alert variant={isDark ? "secondary" : "info"} className="d-flex align-items-center">
                <InfoCircle className="me-2 flex-shrink-0" />
                <div>
                  No assistants match your search criteria. Try different terms or categories.
                </div>
              </Alert>
            ) : (
              <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
