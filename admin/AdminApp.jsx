  ...
  const validateUrl = async (url) => {
    ...
    try {
      ...
      if (data.status.http_code >= 200 && data.status.http_code < 400) {
        ...
        setUrlValidation({
          isValid: false,
          message: `URL returned status: ${data.status.http_code}`,
        });
        return false;
      }
    } catch (error) {
      setUrlValidation({
        isValid: false,
        message: "Could not verify URL accessibility (but URL format is valid)",
      });
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: 20 }}>
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5 }}>
            <input
              name="image"
              placeholder="Image (e.g., assets/img/filename.png)"
              value={form.image}
                ...
                {availableImages.map((imagePath) => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      padding: 5,
                      border: form.image === imagePath ? "2px solid #007bff" : "1px solid #ddd",
                      backgroundColor: "white",
                    }}
                    onClick={() => setForm({ ...form, image: imagePath })}
                  >
                    <img
                      src={`/public/${imagePath}`}
                      alt=""
          ...
          {form.image && (
            <div style={{ marginTop: 5 }}>
              <img
                src={`/public/${form.image}`}
                alt="Preview"
                style={{ width: 100, height: 60, objectFit: "cover", border: "1px solid #ccc" }}
                onError={(e) => {
